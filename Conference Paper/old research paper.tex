...
The LightGBM model proved to be the best performing model with an R² of \num{0.993} and root mean squared error (RMSE) of \num{1137001} bushels, explaining 99.3\% of variance in corn production.
The LightGBM model also performed best without corn acres planted features, achieving an R² score of \num{0.978} with an RMSE of \num{2154832} bushels.
...
Top positively correlated features include \texttt{corn\_acres\_planted} (\(r \approx 0.95+\)) as expected, \texttt{ESoil\_tavg} (\(r = 0.782\)), \texttt{SoilMoi100\_200cm\_inst} (\(r = 0.601\)), \texttt{LWdown\_f\_tavg} (\(r = 0.511\)), \texttt{SoilTMP100\_200cm\_inst} (\(r = 0.454\)), \texttt{Tair\_f\_inst} (\(r = 0.448\)), and \texttt{yield\_per\_acre} (\(r \approx 0.4-0.5\)).
...
PC1 primarily captures scalar information like production amount, dominated by agricultural context (\texttt{corn\_acres\_planted}, \texttt{yield\_per\_acre}) and economic features.
...
Corn acres planted have \num{16}\% missing primarily in early years and smaller counties.
...
XGBoost utilizes gradient boosting with tree learners, employing sequential tree building with gradient optimization and built-in L1 and L2 regularization \cite{chen2016xgboost}.
...
\begin{figure}[htb]
\centering
\includegraphics[width=\columnwidth]{Visuals/xgboost_finetuning_analysis.png}
\caption{XGBoost Fine-Tuning Analysis: Hyperparameter optimization results showing learning curves, parameter sensitivity analysis, or performance metrics across different hyperparameter configurations.}
\label{fig:xgboost_finetuning}
\end{figure}
...
Training employs early stopping with patience=50 rounds, with the best iteration typically occurring at \num{397} out of \num{400} estimators.

\subsubsection{TabNet}

TabNet employs deep learning architecture specifically designed for tabular data, utilizing an attention mechanism for feature selection \cite{arik2021tabnet}.
...
The layer structure consists of input shape (1, 62 features), first LSTM layer with \num{128} units and dropout 0.3, batch normalization, second LSTM layer with 64 units and dropout 0.3, dense layer with 32 units, and output layer with 1 unit.
...
Training configuration includes \num{100} epochs, batch size=256, with callbacks including EarlyStopping with patience=15 and ReduceLROnPlateau with patience=5.00 and factor=0.5.

\subsubsection{Temporal Convolutional Network (TCN)}

The Temporal Convolutional Network employs a flattened input approach for tabular data, utilizing dense layers with L2 regularization and progressive capacity reduction from \num{512} to \num{256} to \num{128} to 64 to 1 unit \cite{bai2018tcn}.
The layer structure begins with input layer flattening (1, num features), followed by four dense blocks: first with \num{512} units and L2 reg=0.001 and Dropout=0.4, second with \num{256} units and L2 reg=0.001 and Dropout=0.4, third with \num{128} units and L2 reg=0.001 and Dropout=0.3, and fourth with 64 units and L2 reg=0.001 and Dropout=0.2.
...
Training configuration includes \num{150} epochs, batch size=256, and callbacks with EarlyStopping (patience=20, min delta=0.0001) and ReduceLROnPlateau (patience=7.00, factor=0.5).
...
Key observations from the performance comparison showed that excluding corn acres planted results in performance degradation with R² decrease of 1.5\% (LightGBM: \num{0.993} → \num{0.978}) and RMSE increase of 89.5\% (\num{1140000} → \num{2150000} bushels).
...
\caption{Model Performance Comparison Without \texttt{corn\_acres\_planted}: Performance metrics for all models when the primary feature is excluded, showing how model performance changes in the reduced feature set configuration.}
\label{fig:model_performance_noacres}
\end{figure}

\begin{figure}[htb]
\centering
\includegraphics[width=\columnwidth]{Visuals/Scattor plot without planted acers.png}
\caption{Predicted vs Actual Values Without \texttt{corn\_acres\_planted}: Scatter plot showing prediction quality for models trained without the most important feature, demonstrating compensatory mechanisms and alternative predictive pathways.}
\label{fig:scatter_noacres}
\end{figure}

\subsection{Why LightGBM Performs Best}

LightGBM achieved the highest R² score (\num{0.993}) and lowest RMSE (\num{1140000} bushels), explaining 99.30\% of variance in corn production.
...
\subsubsection{Feature Importance with corn acres planted}
...
\begin{figure}[htb]
\centering
\includegraphics[width=\columnwidth]{Visuals/Feature Importance.png}
...
\subsubsection{Feature Importance without corn acres planted}
...
\subsubsection{Why fuel cost proxy Becomes Most Important}
...
\caption{Feature Importance Without \texttt{corn\_acres\_planted}: Shift in feature importance when the primary feature is excluded, demonstrating compensatory mechanisms and alternative predictive pathways. Shows how \texttt{fuel\_cost\_proxy} increases to 66.1\% importance.}
\label{fig:feature_importance_noacres}
\end{figure}
...
The R² value of LightGBM reached \num{0.9930} while XGBoost achieved \num{0.9910}.
